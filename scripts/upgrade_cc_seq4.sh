...
for line in open('/tmp/qinstalled.txt'):
    m=re.match(r'^Package ID:\s*([^,]+),\s*Label:\s*tendercc_2\.2', line)
    if m:
        pkg=m.group(1)
        break
print(pkg or '')
...
echo "== APPROVE ORG1 (seq=4)"
peer lifecycle chaincode approveformyorg \
  --channelID "$CHANNEL" --name "$CC_NAME" --version "$CC_VER" \
  --package-id "$PKG_ID" --sequence 4 --collections-config "$CC_COLL" \
  --tls --cafile "$ORDERER_CA" --orderer localhost:7050

echo "== APPROVE ORG2 (seq=4)"
export CORE_PEER_LOCALMSPID=Org2MSP
export CORE_PEER_MSPCONFIGPATH=$PWD/organizations/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
export CORE_PEER_ADDRESS=localhost:9051
export CORE_PEER_TLS_ROOTCERT_FILE=$PWD/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt
...
peer lifecycle chaincode commit \
  --channelID "$CHANNEL" --name "$CC_NAME" --version "$CC_VER" --sequence 4 \
...
echo "== RUN CIVIL FLOW (partial payment + retention) =="
